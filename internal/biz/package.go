...
func (uc *PackageUsecase) UploadPackage(payload []byte) (*model.Package, error) {
	...
	if manifest.Name == model.BuiltinPackageName {
		return nil, fmt.Errorf("package name %q is reserved for the builtin package", manifest.Name)
	}
...
// registerBuiltin packages the bundled trafficgen binary and registers
// it under the builtin identity. A changed binary behind the same
// version (dev builds) replaces the stored payload.
func (uc *PackageUsecase) registerBuiltin(binDir string) error {
	if binDir == "" {
		uc.log.Warn("no bin dir configured; builtin package not registered")

		return nil
	}

	binary, err := os.ReadFile(filepath.Join(binDir, model.BuiltinPackageEntrypoint))
	if err != nil {
		uc.log.Warn("trafficgen binary not found; builtin package not registered", "error", err)
