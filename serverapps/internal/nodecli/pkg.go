...
func PkgMain(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, pkgUsage, agentapi.DefaultRepoPort, agentapi.DefaultPort)
	}
...
func (c *cliCtx) repoBase() (string, error) {
	...
	if base, ok := agent.RememberedRepo(c.dir); ok {
		return base, nil
	}
	...
	return fmt.Sprintf("http://%s", net.JoinHostPort(gw, fmt.Sprint(agentapi.DefaultRepoPort))), nil
}
...
func (c *cliCtx) installed(name, version string) bool {
	return agent.PackageInstalled(c.dir, name, version)
}
