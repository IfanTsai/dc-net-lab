...
func TestHealthCheckNormalise(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			in:   HealthCheck{Type: nodeagentapi.CheckProcess},
			check: func(t *testing.T, got HealthCheck) {
		...
		},
		{
			...
			in:   HealthCheck{Type: nodeagentapi.CheckHTTP, Port: 8080},
			check: func(t *testing.T, got HealthCheck) {
		...
		},
		{
			...
			in:   HealthCheck{Type: nodeagentapi.CheckProcess, Interval: 4 * time.Second, Timeout: 9 * time.Second},
			check: func(t *testing.T, got HealthCheck) {
		...
		{name: "tcp needs port", in: HealthCheck{Type: nodeagentapi.CheckTCP}, wantErr: true},
		{name: "http port out of range", in: HealthCheck{Type: nodeagentapi.CheckHTTP, Port: 70000}, wantErr: true},
	}
...
func TestProbeTCP(t *testing.T) {
	...
	check := HealthCheck{Type: nodeagentapi.CheckTCP, Port: port, Timeout: time.Second}
...
func TestProbeHTTP(t *testing.T) {
	...
	check := HealthCheck{Type: nodeagentapi.CheckHTTP, Port: port, Path: "/healthz", Timeout: time.Second}
...
func TestProbeProcess(t *testing.T) {
	check := HealthCheck{Type: nodeagentapi.CheckProcess, Timeout: time.Second}
...
func TestLivenessMarksHealthy(t *testing.T) {
	...
	spec := withLiveness(shSpec("healthy-svc", "sleep 60", nodeagentapi.RestartNever),
		HealthCheck{Type: nodeagentapi.CheckProcess, Interval: time.Second})
	...
	info := waitHealth(t, m, "healthy-svc", nodeagentapi.HealthHealthy)
	if info.Restarts != 0 {
...
func TestLivenessRestartsUnhealthy(t *testing.T) {
	...
	spec := withLiveness(shSpec("stuck-svc", "sleep 60", nodeagentapi.RestartOnFailure),
		HealthCheck{Type: nodeagentapi.CheckTCP, Port: 1, Interval: time.Second, FailureThreshold: 1})
	if _, err := m.Install(spec); err != nil {
		t.Fatal(err)
	}
...
func TestReadinessReportsServing(t *testing.T) {
	...
	// ever restarting.
	spec := shSpec("web", "sleep 60", nodeagentapi.RestartNever)
	spec.ReadinessCheck = &HealthCheck{Type: nodeagentapi.CheckTCP, Port: port, Interval: time.Second}
	if _, err := m.Install(spec); err != nil {
		t.Fatal(err)
	}
	...
	for {
		...
		if !last.Ready && last.State == nodeagentapi.StateRunning {
			break
		}
...
func TestNoReadinessCheckIsReadyWhenRunning(t *testing.T) {
	...
	if _, err := m.Install(shSpec("plain", "sleep 60", nodeagentapi.RestartNever)); err != nil {
		t.Fatal(err)
	}
...
func TestBootStartsInStartupOrder(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		spec := shSpec(tc.name, "sleep 60", nodeagentapi.RestartNever)
		spec.AutoStart = true
	...
	i1 := waitState(t, m2, "first", nodeagentapi.StateRunning)
	i2 := waitState(t, m2, "second", nodeagentapi.StateRunning)
	i3 := waitState(t, m2, "third", nodeagentapi.StateRunning)
