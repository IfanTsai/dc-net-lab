...
func TestInstallValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "valid", spec: shSpec("app-1", "true", nodeagentapi.RestartNever)},
		{name: "bad name", spec: shSpec("../escape", "true", nodeagentapi.RestartNever), wantErr: true},
		{name: "bad policy", spec: withPolicy(shSpec("app-2", "true", ""), "Sometimes"), wantErr: true},
		{name: "default policy", spec: shSpec("app-3", "true", "")},
		{name: "missing package", spec: withPackage(shSpec("app-4", "true", nodeagentapi.RestartNever), "nosuch", "9.9.9"), wantErr: true},
		{name: "escaping entrypoint", spec: withEntrypoint(shSpec("app-5", "true", nodeagentapi.RestartNever), "../../bin/sh"), wantErr: true},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err == nil && info.State != nodeagentapi.StateConfigured {
...
func TestStartStopLifecycle(t *testing.T) {
	...
	if _, err := m.Install(shSpec("sleeper", "sleep 60", nodeagentapi.RestartNever)); err != nil {
		t.Fatal(err)
	}
	...
	if info.State != nodeagentapi.StateRunning || info.PID == 0 {
	...
	if info.State != nodeagentapi.StateStopped || info.PID != 0 {
...
func TestRestartPolicyAlways(t *testing.T) {
	...
	if _, err := m.Install(shSpec("flappy", "exit 0", nodeagentapi.RestartAlways)); err != nil {
		t.Fatal(err)
	}
...
func TestNeverPolicyFailure(t *testing.T) {
	...
	if _, err := m.Install(shSpec("oneshot", "exit 3", nodeagentapi.RestartNever)); err != nil {
		t.Fatal(err)
	}
	...
	info := waitState(t, m, "oneshot", nodeagentapi.StateFailed)
...
func TestLogsAndRemove(t *testing.T) {
	...
	if _, err := m.Install(shSpec("echoer", "echo one; echo two; sleep 60", nodeagentapi.RestartNever)); err != nil {
		t.Fatal(err)
	}
...
func TestManagerRestoresDesiredState(t *testing.T) {
	...
	if _, err := m1.Install(shSpec("svc", "sleep 60", nodeagentapi.RestartNever)); err != nil {
		t.Fatal(err)
	}
	...
	info := waitState(t, m2, "svc", nodeagentapi.StateRunning)
	if info.PID == 0 {
...
func TestAutoStartStartsOnBoot(t *testing.T) {
	...
	m1 := newManagerAt(t, dir)
	spec := shSpec("enabled-svc", "sleep 60", nodeagentapi.RestartNever)
	spec.AutoStart = true
	...
	info := waitState(t, m2, "enabled-svc", nodeagentapi.StateRunning)
	if info.PID == 0 {
...
func TestOneshotLifecycle(t *testing.T) {
	...
	// with Type=oneshot.
	bad := shSpec("task-bad", "true", nodeagentapi.RestartAlways)
	bad.Type = nodeagentapi.TypeOneshot
	if _, err := m1.Install(bad); err == nil {
		t.Fatal("oneshot with Always restart policy not rejected")
	}

	spec := shSpec("task", "true", nodeagentapi.RestartNever)
	spec.Type = nodeagentapi.TypeOneshot
	if _, err := m1.Install(spec); err != nil {
		t.Fatal(err)
	}
	...
	waitState(t, m1, "task", nodeagentapi.StateExited)
	m1.Shutdown()
	...
	for _, info := range m2.List() {
		if info.Spec.Name == "task" && info.State == nodeagentapi.StateRunning {
...
func TestRemovePackage(t *testing.T) {
	...
	if _, err := m.Install(shSpec("pinner", "true", nodeagentapi.RestartNever)); err != nil {
		t.Fatal(err)
	}
