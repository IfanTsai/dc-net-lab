...
func wireApp(confServer *conf.Server, confData *conf.Data, logger log.Logger, slogLogger *slog.Logger) (*kratos.App, func(), error) {
	...
	operationUsecase := biz.NewOperationUsecase(operationRepo, manager, slogLogger)
	...
	agent := data.NewTrafficAgent(programAgent)
	collector := traffic.NewCollector(trafficStore, agent, driver, trafficUsecase, trafficHistory, slogLogger)
	httpServer := server.NewHTTPServer(confServer, dcNetLabService, terminalUsecase, observerObserver, history, captureUsecase, operationUsecase, trafficUsecase, collector, logger)
	...
	metricsAgent := data.NewMetricsAgent(programAgent)
	metricsCollector := metrics.NewCollector(metricsStore, metricsAgent, driver, history, slogLogger)
	app := newApp(confServer, logger, httpServer, grpcServer, repoServer, observerObserver, metricsCollector, collector, captureManager)
	return app, func() {
		cleanup2()
		cleanup()
	}, nil
