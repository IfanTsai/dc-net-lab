...
func main() {
	...
	// Default to localhost only, per the security requirements. The
	// user-facing origin is the web server (port 8080), which proxies
	// API traffic here.
	flag.StringVar(&sc.HTTPAddr, "listen", "127.0.0.1:8180", "HTTP API address to listen on")
	...
	flag.StringVar(&dc.Runtime, "runtime", "auto", "runtime driver: agent, noop or auto")
	flag.StringVar(&dc.AgentAddr, "agent", runtime.DefaultAgentAddr,
		"data plane agent gRPC address the runtime driver dials")
	flag.StringVar(&dc.BinDir, "bin-dir", "",
		"directory with the builtin package binaries (dcnetlab-trafficgen, dcnetlab-capture); default: the controller binary's directory")
	flag.Parse()
...
func resolveBinDir(dir string, log *slog.Logger) string {
	if dir == "" {
		...
		if err != nil {
			log.Warn("cannot locate controller binary; builtin packages disabled", "error", err)
	...
	if err != nil {
		log.Warn("cannot resolve bin dir; builtin packages disabled", "dir", dir, "error", err)
